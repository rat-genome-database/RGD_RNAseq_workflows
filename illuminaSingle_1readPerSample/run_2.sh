...
matrix_job_id=$(sbatch --export=Run="$run",geo_accession="$geo_accession",PRJdir="$PRJdir",Logdir="$Logdir",AccList="$AccList",BIOProjectID="$BIOProjectID" \
                       ...
                       --error="$Logdir/RSEMmatrix-%j.err" RSEMmatrixs.sh | awk '{print $4}')
...
echo "Matrix and MultiQC completed."

# Update 11 March 2025 streamline process to automatically launch bed and jsons
###################################
