...
# Redirect output to the log file in the scratch directory
...
process_sample() {
    ...
    # CRITICAL FIX: BAM files are in ${scratch_dir}/${sample}/ NOT ${scratch_dir}/${run}/
    # STAR outputs to: ${scratch_dir}/${geo_accession}/${geo_accession}_GENOME_SORT.bam
...
final_output="${scratch_dir}/${BIOProjectID}_sex_result.txt"
mv "$temp_results" "$final_output"

# Copy results to permanent locations
cp "$final_output" "$baseDir/"
cp "${scratch_dir}/${BIOProjectID}_computeSex.txt" "$Logdir/ComputeSex/"

echo ""
echo "Output files:"
echo "  - Sex results: $final_output"
echo "  - Also copied to: $baseDir/${BIOProjectID}_sex_result.txt"
echo "  - Log file copied to: $Logdir/ComputeSex/"
echo ""
