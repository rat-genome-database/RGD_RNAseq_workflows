...
log_file="${Logdir}/RSEM/${BIOProjectID}_RSEMmatrix.out"
exec 1>"$log_file" 2>&1
...
if [[ -z "$AccList" || ! -f "$AccList" ]]; then
  echo "ERROR: AccList not provided or does not exist"
...
while IFS=$'\t' read -r Run geo_accession Tissue Strain Sex PMID GEOpath Title Sample_characteristics StrainInfo; do
    ...
    genes_file="${scratch_dir}/${geo_accession}/${geo_accession}.genes.results"
     ...
     if [ -e "$transcripts_file" ]; then
        ...
        echo "Warning: transcripts results file not found for Sample ${geo_accession}"
    fi

    if [ -e "$genes_file" ]; then
        genes_files+=("$genes_file")
    else
        echo "Warning: Gene results file not found for Sample ${geo_accession}"
    fi
done < "$AccList"
...
if [ ${#transcripts_files[@]} -eq 0 ]; then
    ...
    ${Dependency_dir}/rsem-generate-data-matrix "${transcripts_files[@]}" > "${scratch_dir}/${BIOProjectID}.transcripts.TPM.matrix"
    ${Dependency_dir}/rsem-generate-data-matrix-counts "${transcripts_files[@]}" > "${scratch_dir}/${BIOProjectID}.transcripts.counts.matrix"
...
#    rsem-generate-data-matrix "${genes_files[@]}" > "${scratch_dir}/${BIOProjectID}.genes.TPM.matrix"
    ${Dependency_dir}/rsem-generate-data-matrix "${genes_files[@]}" > "${scratch_dir}/${BIOProjectID}.genes.TPM.matrix"
    ${Dependency_dir}/rsem-generate-data-matrix-counts "${genes_files[@]}" > "${scratch_dir}/${BIOProjectID}.genes.counts.matrix"
fi
...
cp "${scratch_dir}/${BIOProjectID}.transcripts.counts.matrix" $baseDir
cp "${scratch_dir}/${BIOProjectID}.genes.counts.matrix" $baseDir
