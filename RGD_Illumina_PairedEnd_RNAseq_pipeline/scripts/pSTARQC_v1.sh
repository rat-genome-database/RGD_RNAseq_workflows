...
# Iterate Accession list; skip header
tail -n +2 "$uniqueAccList" | while IFS=$'\t' read -r Run geo_accession _rest; do
  [[ -z "$geo_accession" ]] && continue

  wrkDir="${scratch_dir}/${Run}"
...
#  star_log="${wrkDir}/${geo_accession}_STARLog.final.out"
  star_log="$baseDir/reads_fastq/${geo_accession}/log_files/STARQC/${geo_accession}_STARLog.final.out"
  if [[ ! -f "$star_log" ]]; then
