...
#18 June 2025 removed form header to reduce redundant log files: #SBATCH --output=%x-%j.out #SBATCH --error=TxBed.err
...
#mkdir -p "${Logdir}/${geo_accession}"
mkdir -p "${PRJdir}/${geo_accession}"

# Redirect output to the log file in the project directory after it is created
#log_file="${Logdir}/${geo_accession}/${geo_accession}_TxBedTPM.out"
log_file="${PRJdir}/${geo_accession}/${geo_accession}_TxBedTPM.out"
exec 1>"$log_file" 2>&1
...
# Check if the isoforms results input file exists
tx_file="${scratch_dir}/${Run}/${geo_accession}.isoforms.results"
if [ -f "$tx_file" ]; then
    echo "Isoforms file exists. Proceeding to create JSON."

    output_file="${scratch_dir}/${Run}/${geo_accession}_TX_TPMs.txt"

    # Extract transcript IDs and TPMs from the isoforms results file
    ...
    sorted_file="${scratch_dir}/${Run}/${geo_accession}_sorted_TX_TPMs.txt"
...
#        cat "${Logdir}/${Run}/${geo_accession}_sort_error.log" >> "$log_file"
        cat "${PRJdir}/${Run}/${geo_accession}_sort_error.log" >> "$log_file"
        exit 1
    fi
    ...
    if [ -f "$TBed" ]; then
        merged_file="${scratch_dir}/${Run}/${geo_accession}_merged_TX.bed"
    ...
    rawOP="${scratch_dir}/${Run}/${geo_accession}_TX_rawTPM.bed"
    ...
    chr0_filtered="${scratch_dir}/${Run}/${geo_accession}_TX_TPMfiltchr0.bed"
