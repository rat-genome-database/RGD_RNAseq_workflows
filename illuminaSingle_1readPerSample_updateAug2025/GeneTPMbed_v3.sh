...
log_file="${PRJdir}/${geo_accession}/log_files/${geo_accession}_GeneBedTPM.out"
exec 1>"$log_file" 2>&1
...
#    sort -k1,1 "$output_file" > "$sorted_file" 2>"${Logdir}/${geo_accession}/log_files/${geo_accession}_sort_error.log"
    sort -k1,1 "$output_file" > "$sorted_file" 2>"${PRJdir}/${geo_accession}/log_files/${geo_accession}_sort_error.log"
    if [ $? -ne 0 ]; then
        echo "Error: Failed to sort $output_file" >> "$log_file"
#        cat "${Logdir}/${Run}/${geo_accession}_sort_error.log" >> "$log_file"
        cat "${PRJdir}/${geo_accession}/log_files/${geo_accession}_sort_error.log" >> "$log_file"
        exit 1
    fi
...
#    sort -k1,1 -k2,2n -k3,3n "$chr0_filtered" > "$CHRsorted_file" 2>>"${Logdir}/${geo_accession}/${geo_accession}_CHRsort_error.log"
    sort -k1,1 -k2,2n -k3,3n "$chr0_filtered" > "$CHRsorted_file" 2>>"${PRJdir}/${geo_accession}/log_files/${geo_accession}_CHRsort_error.log"
...
#        cat "${Logdir}/${Run}/${geo_accession}_CHRsort_error.log" >> "$log_file"
        cat "${PRJdir}/${geo_accession}/log_files/${geo_accession}_CHRsort_error.log" >> "$log_file"
        exit 1
    fi
