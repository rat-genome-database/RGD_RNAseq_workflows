...
log_file="${PRJdir}/${geo_accession}/log_files/${geo_accession}_TxBedTPM.out"
exec 1>"$log_file" 2>&1
...
if [ -f "$tx_file" ]; then
    ...
    sort -k1,1 "$output_file" > "$sorted_file" 2>"${PRJdir}/${geo_accession}/log_files/${geo_accession}_txsort_error.log" 
   if [ $? -ne 0 ]; then
...
#        cat "${PRJdir}/${Run}/${geo_accession}_txsort_error.log" >> "$log_file"
        cat "${PRJdir}/${geo_accession}/log_files/${geo_accession}_txsort_error.log" >> "$log_file"
        exit 1
    fi
...
#    CHRsorted_file="${Logdir}/${geo_accession}/RNAseq_${unique_name}_TXTPMfinalOP.bed" ##try sending to scratch and only final to home
    ...
    sort -k1,1 -k2,2n -k3,3n "$chr0_filtered" > "$CHRsorted_file" 2>>"${PRJdir}/${geo_accession}/log_files/${geo_accession}_txCHRsort_error.log"
...
#        cat "${Logdir}/${Run}/${geo_accession}_CHRsort_error.log" >> "$log_file"
        cat "${PRJdir}/${geo_accession}/log_files/${geo_accession}_txCHRsort_error.log" >> "$log_file"
        exit 1
    fi
