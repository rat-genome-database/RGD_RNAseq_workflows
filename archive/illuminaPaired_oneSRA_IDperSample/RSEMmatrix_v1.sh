...
while IFS=$'\t' read -r Run geo_accession Tissue Strain Sex PMID GEOpath Title Sample_characteristics StrainInfo; do
    ...
    isoforms_file="${scratch_dir}/${Run}/${geo_accession}.isoforms.results"
...
cp "${scratch_dir}/${BIOProjectID}.isoforms.TPM.matrix" $FinalOPdir
...
##################################
sh /home/wdemos/STAR_RSEM_pipeline/illuminaPaired_oneSRA_IDperSample/ConflictedSampleReport.sh ${BIOProjectID}
