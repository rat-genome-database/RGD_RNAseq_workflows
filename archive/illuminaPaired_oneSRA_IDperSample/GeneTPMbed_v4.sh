...
if [ -f "$gene_file" ]; then
    ...
    sort -k1,1 "$output_file" > "$sorted_file" 2>"${PRJdir}/${geo_accession}/log_files/${geo_accession}_gsort_error.log"
    if [ $? -ne 0 ]; then
        echo "Error: Failed to sort $output_file" >> "$log_file"
        cat "${PRJdir}/${geo_accession}/log_files/${geo_accession}_gsort_error.log" >> "$log_file"
        exit 1
    fi
    ...
    sort -k1,1 -k2,2n -k3,3n "$chr0_filtered" > "$CHRsorted_file" 2>>"${PRJdir}/${geo_accession}/log_files/${geo_accession}_gCHRsort_error.log"

    # Check if sorting succeeded
    if [ $? -ne 0 ]; then
        echo "Error: Sort failed for $chr0_filtered" >> "$log_file"
        cat "${PRJdir}/${geo_accession}/log_files/${geo_accession}_gCHRsort_error.log" >> "$log_file"
        exit 1
    fi
