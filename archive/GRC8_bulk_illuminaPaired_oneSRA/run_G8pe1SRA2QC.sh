#!/bin/bash
#SBATCH --job-name=SRAtoQCp1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=12
#SBATCH --mem-per-cpu=4gb
#SBATCH --time=12:00:00
#SBATCH --account=akwitek
#SBATCH --partition=normal
#SBATCH --output=%x-%j.out
#SBATCH --mail-type=FAIL
#SBATCH --mail-user=wdemos@mcw.edu
#SBATCH --error=SRAtoQCp1

# Enable debugging by printing each command before execution
set -x

# Check command line arguments
if [ $# -ne 2 ]; then
    echo "Usage: $0 <SRA_accession_list.txt> <BioProject_ID>"
    exit 1
fi

AccList=$1 
BIOProjectID=$2
myDir="/home/wdemos"
PRJdir="$myDir/data/expression/GEO/$BIOProjectID/reads_fastq"
Logdir="$myDir/data/expression/GEO/$BIOProjectID/log_files"
screenconfig="$myDir/dependencies/FastQ_Screen_Genomes/fastq_screen.conf"
scratchDir="/scratch/g/akwitek/wdemos/$BIOProjectID"
baseDir="$myDir/data/expression/GEO/$BIOProjectID"

# Status tracking
SUCCESS_DIR="$baseDir/.status"
mkdir -p "$SUCCESS_DIR"

# Create directories
mkdir -p "$PRJdir"
mkdir -p "$Logdir"

# Redirect output to a master log file
log_file="${Logdir}/${BIOProjectID}_Step1_SRAtoQC.out"
exec 1>"$log_file" 2>&1

echo "===== STEP 1: SRA to QC Pipeline ====="
echo "BioProject: $BIOProjectID"
echo "AccList: $AccList"
echo "Started: $(date)"
echo "=========================================="

# Read accessions and submit jobs
job_ids=()  # Array to store job IDs
declare -A sample_map  # Map job_id -> Run accession

while IFS=$'\t' read -r Run geo_accession Tissue Strain Sex PMID GEOpath Title Sample_characteristics StrainInfo; do
    # Skip the header row
    if [ "$Run" == "Run" ]; then
        continue
    fi
    
    # Create a unique subdirectory for each geo_accession under reads_fastq
    geo_smp_dir="${PRJdir}/${geo_accession}"
    mkdir -p "$geo_smp_dir"  # Ensure the directory exists
    geo_log_dir="${geo_smp_dir}/log_files"
    mkdir -p "$geo_log_dir"  # Ensure the directory exists
    
    # Submit each sample as a separate job and capture the job ID
    job_id=$(sbatch --export=BIOProjectID="$BIOProjectID",Run="$Run",geo_accession="$geo_accession",PRJdir="$PRJdir",screenconfig="$screenconfig",SUCCESS_DIR="$SUCCESS_DIR" \
        --output="${geo_log_dir}/SRA2QC-%j.out" \
        --error="${geo_log_dir}/SRA2QC-%j.err" \
        SRA2QC_v4.sh | awk '{print $4}')
    
    # Store the job ID in the array
    job_ids+=("$job_id")
    sample_map["$job_id"]="$Run"
    
    echo "Submitted job $job_id for sample $Run ($geo_accession)"
done < "$AccList"

total_samples=${#job_ids[@]}
echo "All $total_samples jobs submitted."
echo "Job IDs: ${job_ids[@]}"

# Wait for all jobs to complete
echo "Waiting for all sample jobs to complete..."
for job_id in "${job_ids[@]}"; do
    while squeue -u $USER | grep -q "$job_id"; do
        sleep 5
    done
done

echo "All sample processing jobs have finished."

# Check for failures by looking for success markers
echo "Checking for failed samples..."
failed_samples=()
succeeded_samples=()

while IFS=$'\t' read -r Run geo_accession Tissue Strain Sex PMID GEOpath Title Sample_characteristics StrainInfo; do
    # Skip the header row
    if [ "$Run" == "Run" ]; then
        continue
    fi
    
    # Check for success marker
    success_marker="${SUCCESS_DIR}/.success_${Run}"
    
    if [ -f "$success_marker" ]; then
        succeeded_samples+=("$Run")
    else
        failed_samples+=("$Run")
    fi
done < "$AccList"

echo "=========================================="
echo "Sample Processing Summary:"
echo "Total samples: $total_samples"
echo "Succeeded: ${#succeeded_samples[@]}"
echo "Failed: ${#failed_samples[@]}"

if [ ${#failed_samples[@]} -gt 0 ]; then
    echo "=========================================="
    echo "FAILED SAMPLES:"
    for sample in "${failed_samples[@]}"; do
        echo "  - $sample"
    done
    echo "=========================================="
    echo "ERROR: One or more samples failed processing."
    echo "STEP 1 FAILED for $BIOProjectID"
    echo "Completed: $(date)"
    exit 1
fi

echo "All samples processed successfully!"
echo "=========================================="

# Load MultiQC and run it on the PRJdir
echo "Running MultiQC..."
module load python/3.9.1 multiqc/1.18
python3 -m multiqc "$scratchDir" -o "${baseDir}" -n ${BIOProjectID}_fastq_multiQC_report || { 
    echo "MultiQC failed"
    echo "STEP 1 FAILED for $BIOProjectID"
    echo "Completed: $(date)"
    exit 1
}
module unload python/3.9.1 multiqc/1.18

echo "MultiQC completed successfully."
echo "=========================================="
echo "STEP 1 COMPLETED SUCCESSFULLY for $BIOProjectID"
echo "Completed: $(date)"
echo "=========================================="

# Create overall success marker for this bioproject
touch "${SUCCESS_DIR}/.step1_complete"

exit 0
