...
log_file="${Logdir}/${geo_accession}_RSEM.out"
exec 1>"$log_file" 2>&1
...
# rm "${scratch_dir}/${geo_accession}/${geo_accession}.transcript.bam"
# rm "${scratch_dir}/${Run}/${geo_accession}_STARAligned.toTranscriptome.out.bam"
 cp "$tempOPdir/$geo_accession.isoforms.results" $PRJdir/${geo_accession}
 cp "$tempOPdir/$geo_accession.genes.results" $PRJdir/${geo_accession}
