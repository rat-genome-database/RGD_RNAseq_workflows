...
# 18 June 2025 removed form header to reduce redundant log files: #SBATCH --output=%x-%j.out #SBATCH --error=TXjson.err
...
echo "transcript bed json generations 'Run' variable is ${Run}"
...
#log_file="${PRJdir}/${geo_accession}/log_files/bedsNjsons/${geo_accession}_TxBedJSON.out"
#exec 1>"$log_file" 2>&1
...
# Bed file is not  necessary to generate file. Will run a clean up script and check for files
TXbed_file="${PRJdir}/${sample}/RNAseq_${unique_name}_TXTPMfinalOP.bed.gz"
echo "Generating JSON for $TXbed_file"
    ...
    json_content=$(cat <<EOF
{
  ...
  "adapter": {
    ...
    "bedGzLocation": {
      ...
      "uri":  "json/RNAseq_${unique_name}_TXTPMfinalOP.bed.gz"
    },
    "index": {
      "location": {
        "locationType": "UriLocation",
        "uri": "json/RNAseq_${unique_name}TXTPMfinalOP.bed.gz.tbi"
      }
    }
  },
  "displays": [
