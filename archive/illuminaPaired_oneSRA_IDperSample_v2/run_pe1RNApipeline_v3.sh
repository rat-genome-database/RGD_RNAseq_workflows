...
starqc_output=$(sbatch \
    ...
    pSTARQC_v1.sh "$AccList" "$BIOProjectID" )

# Extract and validate job ID
starqc_job_id=$(echo "$starqc_output" | awk '{print $4}')
echo "pSTARQC_v1.sh job submission output: $starqc_output"
...
echo "pSTARQC_v1.sh completed. Output expected at: ${STARQC_log_dir}/${BIOProjectID}_STAR_Align_sum.txt"
...
job_output=$(sbatch \
    --job-name="computeSex" \
    --export=PRJdir="$PRJdir",Logdir="$Logdir",AccList="$AccList",BIOProjectID="$BIOProjectID",baseDir="$baseDir" \
    --output="$ComputeSex_log_dir/ComputeSex-%j.out" \
    --error="$ComputeSex_log_dir/ComputeSex-%j.err" \
    ComputeSex_v4.sh "$AccList" "$BIOProjectID")
...
if [ -f "$scratch_dir/rsemref.chrlist" ] && [ -f "$scratch_dir/rsemref.grp" ] && [ -f "$scratch_dir/rsemref.idx.fa" ] && [ -f "$scratch_dir/rsemref.n2g.idx.fa" ] && [ -f "$scratch_dir/rsemref.seq" ] && [ -f "$scratch_dir/rsemref.ti" ] && [ -f "$scratch_dir/rsemref.transcripts.fa" ]; then
    ...
    echo "Submitting RSEMref_v4.sh to generate the RSEM reference."
    rsemRef_job_id=$(sbatch --export=BIOProjectID="$BIOProjectID" \
                           --output="$Rref_log_dir/RSEMref-%j.out" \
                           --error="$Rref_log_dir/RSEMref-%j.err" RSEMref_v4.sh | awk '{print $4}')
    if [ -z "$rsemRef_job_id" ]; then echo "Error: RSEMref_v4.sh submission failed."; exit 1; fi
...
job_id_output=$(sbatch --export=Run="$run",geo_accession="$geo_accession",PRJdir="$PRJdir",Logdir="$Logdir",AccList="$AccList",BIOProjectID="$BIOProjectID",baseDir="$baseDir" \
              --output="$matrix_log_dir/RSEMmatrix-%j.out" \
              --error="$matrix_log_dir/RSEMmatrix-%j.err" RSEMmatrix_v4.sh 2>&1)

matrix_job_id=$(echo "$job_id_output" | awk '{print $4}')
echo "RSEMmatrix_v4.sh submission output: $job_id_output" >> "$log_file"
...
echo "RSEMmatrix_v4.sh submitted with job ID: $matrix_job_id" >> "$log_file"
...
while IFS=$'\t' read -r run geo_accession Tissue Strain Sex PMID GEOpath Title Sample_characteristics StrainInfo; do
    ...
    for job_type in "2bigWigjson" "geneJson" "txBedJson" "GeneTPMbed" "txTPMbed"; do
        case "$job_type" in
            "2bigWigjson") script_name="BWjson_v4.sh" ;;
            "geneJson") script_name="gJSON_v4.sh" ;;
            "txBedJson") script_name="tJSON_v4.sh" ;;
            "GeneTPMbed") script_name="GeneTPMbed_v4.sh" ;;
            "txTPMbed") script_name="txTPMbed_v4.sh" ;;
        esac
