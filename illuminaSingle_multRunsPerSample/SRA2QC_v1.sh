...
while [ "$attempt" -le "$max_attempts" ]; do
  ...
  if fasterq-dump.3.1.1 "$scratch_dir/$Run" --threads 12 --outdir "$scratch_dir/$Run" -t "${scratch_dir}" -x; then
    fastq1="${scratch_dir}/${Run}/${Run}.fastq" 

    if [ -f "$fastq1" ]; then
      echo "Renaming fastq file to GEO accession: $geo_accession"
      new_fastq1="${scratch_dir}/${Run}/${geo_accession}_${Run}.fastq"
      ...
      echo "Compress fastq file"
      pigz "$new_fastq1" || { echo "Compression failed"; exit 1; }
      ...
      echo "FASTQ file not found for $Run on attempt $attempt"
    fi
