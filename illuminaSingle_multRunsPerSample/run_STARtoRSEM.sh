...
if [ ! -d "$INDEX_DIR" ] || [ ! -f "$INDEX_DIR/SA" ] || [ ! -f "$INDEX_DIR/SAindex" ] || [ ! -f "$INDEX_DIR/Genome" ]; then
    echo "Submitting starRef_v1.sh to generate the STAR reference."
    starRef_job_id=$(sbatch --export=PRJdir="$PRJdir",Logdir="$Logdir",BIOProjectID="$BIOProjectID",length="$length" \
                        ...
                        --error="$StarRef_log_dir/starRef-%j.err" starRef_v1.sh | awk '{print $4}')
    if [ -z "$starRef_job_id" ]; then
        echo "Error: starRef_v1.sh submission failed."
        exit 1
    fi
    echo "STAR reference job ID: $starRef_job_id"
else
    echo "STAR reference already exists. Skipping starRef_v1.sh."
    starRef_job_id=""
fi
...
job_output=$(sbatch \
    ...
    ComputeSex_v1.sh "$AccList" "$BIOProjectID")
#    ComputeSex_v1.sh "$uniqueAccList" "$BIOProjectID")

# Extract and validate job ID
compute_sex_job_id=$(echo "$job_output" | awk '{print $4}')
echo "ComputeSex_v1 job submission output: $job_output"
...
echo "ComputeSex_v1.sh submitted with job ID: $compute_sex_job_id"

# Wait for the ComputeSex job to complete
wait_for_jobs "$compute_sex_job_id"
echo "ComputeSex_v1 job completed. Continuing to next step..."
##############################
...
if [ -f "$scratch_dir/rsemref.chrlist" ] && [ -f "$scratch_dir/rsemref.grp" ] && [ -f "$scratch_dir/rsemref.idx.fa" ] && [ -f "$scratch_dir/rsemref.n2g.idx.fa" ] && [ -f "$scratch_dir/rsemref.seq" ] && [ -f "$scratch_dir/rsemref.ti" ] && [ -f "$scratch_dir/rsemref.transcripts.fa" ]; then
    ...
    rsemRef_job_id=$(sbatch --export=BIOProjectID="$BIOProjectID" \
                           ...
                           --error="$rref_log_dir/RSEMref-%j.err" RSEMref_v1.sh | awk '{print $4}')
    if [ -z "$rsemRef_job_id" ]; then echo "Error: RSEMref_v1.sh submission failed."; exit 1; fi
...
matrix_job_id=$(sbatch \
    ...
    RSEMmatrix_v1.sh | awk '{print $4}')

if [ -z "$matrix_job_id" ]; then
    echo "Error: RSEMmatrix_v1.sh submission failed."
    exit 1
fi

echo "RSEMmatrix_v1.sh submitted with job ID: $matrix_job_id (PASS-only)."
