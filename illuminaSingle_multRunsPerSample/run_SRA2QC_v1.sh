...
log_file="${Logdir}/${BIOProjectID}_SEM_SRA2QC.out"
exec 1>"$log_file" 2>&1
...
while IFS=$'\t' read -r Run geo_accession Tissue Strain Sex PMID GEOpath Title Sample_characteristics StrainInfo; do
    ...
    job_id=$(sbatch --export=BIOProjectID="$BIOProjectID",Run="$Run",geo_accession="$geo_accession",screenconfig="$screenconfig" \
        --output="${geo_log_dir}/SRA2QC-%j.out" \
        --error="${geo_log_dir}/SRA2QC-%j.err" \
        SRA2QC_v1.sh | awk '{print $4}')
