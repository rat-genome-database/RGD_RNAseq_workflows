...
#18 June 2025 removed #SBATCH --error=SRAtoQCerr from header and changed.err and .out to reduce log files
...
while IFS=$'\t' read -r Run geo_accession Tissue Strain Sex PMID GEOpath Title Sample_characteristics StrainInfo; do
    ...
    geo_log_dir="${PRJdir}/${geo_accession}"
    ...
    job_id=$(sbatch --export=BIOProjectID="$BIOProjectID",Run="$Run",geo_accession="$geo_accession",PRJdir="$PRJdir",screenconfig="$screenconfig" \
        --output="${geo_log_dir}/SRA2QC-%j.log" \
        --error="${geo_log_dir}/SRA2QC-%j.log" \
...
python3 -m multiqc "$scratchDir" -o "${Logdir}" -n ${BIOProjectID}_fastq_multiQC_report || { echo "MultiQC failed"; exit 1; }
