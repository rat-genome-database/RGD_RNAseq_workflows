...
#18 June removed form header: #SBATCH --output=%x-%j.out, #SBATCH --error=geneBed.err
...
log_file="${PRJdir}/${geo_accession}/${geo_accession}_GeneBedTPM.out"
exec 1>"$log_file" 2>&1
...
# Check if the gene results input file exists
gene_file="${scratch_dir}/${Run}/${geo_accession}.genes.results"
if [ -f "$gene_file" ]; then
    echo "Gene file exists. Proceeding to create JSON."

    output_file="${scratch_dir}/${Run}/${geo_accession}_Genes_TPMs.txt"

    # Extract gene IDs and TPMs from the gene results file
    echo "Parsing GENE name and TPMs from $gene_file"
    awk 'BEGIN {OFS="\t"} FNR > 1 {print $1, $6}' "$gene_file" > "$output_file"
    if [ $? -ne 0 ]; then
        echo "Error: Failed to parse $gene_file" >> "$log_file"
        exit 1
    fi
    ...
    # Sort the TPM output file by gene ID
    sorted_file="${scratch_dir}/${Run}/${geo_accession}_sorted_Genes_TPMs.txt"
    echo "Sorting $output_file"
#    sort -k1,1 "$output_file" > "$sorted_file" 2>"${Logdir}/${geo_accession}/${geo_accession}_sort_error.log"
    sort -k1,1 "$output_file" > "$sorted_file" 2>"${PRJdir}/${geo_accession}/${geo_accession}_sort_error.log"
    if [ $? -ne 0 ]; then
        echo "Error: Failed to sort $output_file" >> "$log_file"
#        cat "${Logdir}/${Run}/${geo_accession}_sort_error.log" >> "$log_file"
        cat "${PRJdir}/${geo_accession}/${geo_accession}_sort_error.log" >> "$log_file"
        exit 1
    fi
    ...
    # Merge with the reference BED file by gene name
    if [ -f "$GBed" ]; then
        merged_file="${scratch_dir}/${Run}/${geo_accession}_merged_GENE.bed"
        echo "Merging $sorted_file with $GBed"
        awk 'BEGIN {OFS="\t"} NR==FNR {a[$1]=$2; next} $4 in a {print $1, $2, $3, $4, a[$4]}' "$sorted_file" "$GBed" > "$merged_file"
        if [ $? -ne 0 ]; then
            echo "Error: Failed to merge $sorted_file with $GBed" >> "$log_file"
            exit 1
        fi
    else
        echo "Error: $GBed does not exist" >> "$log_file"
        exit 1
    fi

    # Generate the raw output file with all TPM values
    rawOP="${scratch_dir}/${Run}/${geo_accession}_GENE_rawTPM.bed"
    ...
    chr_filtered="${scratch_dir}/${Run}/${geo_accession}_NWfiltered.bed"
    chr0_filtered="${scratch_dir}/${Run}/${geo_accession}_GENE_TPMfiltchr0.bed"
...
#    CHRsorted_file="${Logdir}/${geo_accession}/RNAseq_${unique_name}_geneTPMfinalOP.bed"
...
#    sort -k1,1 -k2,2n -k3,3n "$chr0_filtered" > "$CHRsorted_file" 2>>"${Logdir}/${geo_accession}/${geo_accession}_CHRsort_error.log"
    sort -k1,1 -k2,2n -k3,3n "$chr0_filtered" > "$CHRsorted_file" 2>>"${PRJdir}/${geo_accession}/${geo_accession}_CHRsort_error.log"

    # Check if sorting succeeded
    if [ $? -ne 0 ]; then
        echo "Error: Sort failed for $chr0_filtered" >> "$log_file"
#        cat "${Logdir}/${Run}/${geo_accession}_CHRsort_error.log" >> "$log_file"
        cat "${PRJdir}/${geo_accession}/${geo_accession}_CHRsort_error.log" >> "$log_file"
        exit 1
    fi
    ...
    echo "Gene TPM BED file processing is complete for $geo_accession" >> "$log_file"
    end_time=$(date +%s)
    runtime=$((end_time - start_time))
    echo "Processing time: $runtime seconds" >> "$log_file"

else
    echo "Error: Gene results file $gene_file does not exist" >> "$log_file"
    exit 1
fi
