...
#18June2025 eliminated form header to reduce redundant log files #SBATCH --output=%x-%j.out #SBATCH --error=SRA2QC.err
...
###
#updated 5/27 from --mem-per-cpu=1gb
#updated 5/27 from --time=03:00:00
####
...
#if download fails because the size is larger than 20G
prefetch -O "$scratch_dir" "$Run" --max-size 30G || { echo "Prefetch failed for $Run"; exit 1; }
...
while [ $attempt -le $max_attempts ]; do
    ...
    if fasterq-dump.3.1.1 "$scratch_dir/$Run" --threads 12 --outdir "$scratch_dir/$Run" -t /scratch/g/akwitek/wdemos/$BIOProjectID -x; then
        ...
        if [ -f "$fastq1" ] && [ -f "$fastq2" ]; then 
            ...
            new_fastq1="${scratch_dir}/${Run}/${geo_accession}_1.fastq"
            new_fastq2="${scratch_dir}/${Run}/${geo_accession}_2.fastq"
            mv "$fastq1" "$new_fastq1"
            mv "$fastq2" "$new_fastq2"

            if [ -f "$fastq3" ]; then
                new_fastq3="${scratch_dir}/${Run}/${geo_accession}.fastq"
            ...
            # Create success marker to indicate this sample completed successfully
            if [ -n "$SUCCESS_DIR" ] && [ -n "$Run" ]; then
                touch "${SUCCESS_DIR}/.success_${Run}"
                echo "Created success marker for sample $Run"
            fi
