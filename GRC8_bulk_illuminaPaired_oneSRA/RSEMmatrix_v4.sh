...
if [ ${#transcripts_files[@]} -eq 0 ]; then
    ...
    /home/wdemos/dependencies/rsem-generate-data-matrix-counts "${transcripts_files[@]}" > "${scratch_dir}/${BIOProjectID}.transcripts.counts.matrix"
...
#    rsem-generate-data-matrix "${genes_files[@]}" > "${scratch_dir}/${BIOProjectID}.genes.TPM.matrix"
    ...
    /home/wdemos/dependencies/rsem-generate-data-matrix-counts "${genes_files[@]}" > "${scratch_dir}/${BIOProjectID}.genes.counts.matrix"
fi
...
cp "${scratch_dir}/${BIOProjectID}.transcripts.counts.matrix" $FinalOPdir
cp "${scratch_dir}/${BIOProjectID}.genes.counts.matrix" $FinalOPdir
...
python3 -m multiqc "$scratch_dir" -o "${FinalOPdir}" -n ${BIOProjectID}_final_multiQC_report || { echo "MultiQC failed"; exit 1; }

##################################
# Run Sample Sex Conflict Script #
##################################
sh /home/wdemos/STAR_RSEM_pipeline/GRC8_bulk_illuminaPaired_oneSRA/ConflictedSampleReport_v4.sh ${BIOProjectID}
